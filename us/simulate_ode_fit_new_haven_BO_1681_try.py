...
import setproctitle
setproctitle.setproctitle('New Haven_SEIR@hanzhenyu')
...
def setup_args(parser=None):
    ...
    # Default Params
    parser.add_argument('--population_data', default='./population/new_haven_pop.npy', help='Loc of pop data')
    parser.add_argument('--cases_data', default='./cases/new_haven.npy', help='Loc of cases data')
    parser.add_argument('--units', default=1681, help='Unit Num', type=int)
...
def multi_process_fit(i,fit_epoch):
    output_dir = Path('./simulated_results_New_Haven_1681/')
    ...
    cases_data_first = cases_data[60:70-6+7]
    ...
    while error > 0.5:
        ...
        pbounds = {'pi': (0, 0.001), 'early_detect': (0, 1), 'mobility': (0, 0.0003)}
    ...
    while error > 0.5:
        error = 0
        pbounds = { 'pi': (0.0003, 0.006), 'early_detect': (0, 1), 'mobility': (optimizer1.max['params']['mobility'],0.0003)}
