...
import setproctitle
setproctitle.setproctitle('Harris_SEIR@hanzhenyu')
...
def setup_args(parser=None):
    ...
    # Default Params
    parser.add_argument('--population_data', default='./population/harris_pop.npy', help='Loc of pop data')
    parser.add_argument('--cases_data', default='./cases/harris.npy', help='Loc of cases data')
    parser.add_argument('--units', default=1369, help='Unit Num', type=int)
...
def multi_process_fit(i,fit_epoch):
    output_dir = Path('./simulated_results_Harris_1369/')
    ...
    cases_data_first = cases_data[60:70-6+14]
    ...
    while error > 0.5:
        error = 0
        pbounds = { 'pi': (0, 0.001), 'early_detect': (0, 1), 'mobility': (optimizer1.max['params']['mobility'],0.0003)}
