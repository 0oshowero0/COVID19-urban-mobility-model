...
import setproctitle
setproctitle.setproctitle('Los_Angeles_SEIR@hanzhenyu')
...
def setup_args(parser=None):
    ...
    # Default Params
    parser.add_argument('--population_data', default='./population/los_angeles_pop.npy', help='Loc of pop data')
    parser.add_argument('--cases_data', default='./cases/los_angeles.npy', help='Loc of cases data')
    parser.add_argument('--units', default=784, help='Unit Num', type=int)
...
def multi_process_fit(i,fit_epoch):
    output_dir = Path('./simulated_results_Los_Angeles_784/')
    ...
    cases_data_first = cases_data[55:70-6+7]
    ...
    while error > 0.5:
        error = 0
        pbounds = { 'pi': (0, 0.001), 'early_detect': (0, 1), 'mobility': (0,optimizer1.max['params']['mobility'])}
