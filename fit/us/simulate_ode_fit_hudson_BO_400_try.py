...
import setproctitle
setproctitle.setproctitle('Hudson_SEIR@hanzhenyu')
...
def setup_args(parser=None):
    ...
    # Default Params
    parser.add_argument('--population_data', default='./population/hudson_pop.npy', help='Loc of pop data')
    parser.add_argument('--cases_data', default='./cases/hudson.npy', help='Loc of cases data')
    parser.add_argument('--units', default=400, help='Unit Num', type=int)
...
def multi_process_fit(i,fit_epoch):
    output_dir = Path('./simulated_results_Hudson_400/')
    ...
    while error > 0.5:
        error = 0
        pbounds = { 'pi': (0, 0.0006),'early_detect': (0, 1), 'mobility': (optimizer1.max['params']['mobility'],0.0003)}
