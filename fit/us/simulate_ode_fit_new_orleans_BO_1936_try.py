...
import setproctitle
setproctitle.setproctitle('New Orleans_SEIR@hanzhenyu')
...
def setup_args(parser=None):
    ...
    # Default Params
    parser.add_argument('--population_data', default='./population/new_orleans_pop.npy', help='Loc of pop data')
    parser.add_argument('--cases_data', default='./cases/new_orleans.npy', help='Loc of cases data')
    parser.add_argument('--units', default=1936, help='Unit Num', type=int)
...
def multi_process_fit(i,fit_epoch):
    output_dir = Path('./simulated_results_New_Orleans_1936/')
    ...
    while error > 0.5:
        ...
        pbounds = {'pi': (0.0003, 0.004), 'early_detect': (0, 1), 'mobility': (0, 0.0003)}
    ...
    while error > 0.5:
        error = 0
        pbounds = { 'pi': (0.0003, 0.004) , 'early_detect': (0, 1), 'mobility': (0,optimizer1.max['params']['mobility'])}
